...
import com.okta.mongodb.GeneradoScripts.model.body.balanceoNth.LineInterfacesBody;
import com.okta.mongodb.GeneradoScripts.model.body.balanceoNth.BalanceoNthBody;

@RestController
@RequestMapping("/balanceo-nth")
public class BalanceoNthController {

    @PostMapping
    public ResponseEntity<?> create(@RequestBody BalanceoNthBody balanceoNthBody) {
        System.out.println("==============================");
        System.out.println(balanceoNthBody.getLinea());
        System.out.println(balanceoNthBody.getLocal());
        System.out.println(balanceoNthBody.getRouter());
        System.out.println(balanceoNthBody.getInterfaceTarget());


        for (LineInterfacesBody inter : balanceoNthBody.getInterfaces()) {
        ...
        String comentario = """
                ...
                # Ultima Actualización | Update Script: 2025
                # Createdo: 2bytec.com
        ...
        /routing table
                        """;
        if (balanceoNthBody.getRouter().equals("v7")) {
            for (LineInterfacesBody inter : balanceoNthBody.getInterfaces()) {
        ...
        if (balanceoNthBody.getLocal().equals("local-ip")) {
        ...
        if (balanceoNthBody.getLocal().equals("local-ip1")) {
            ...
            for (LineInterfacesBody inter : balanceoNthBody.getInterfaces()) {
                mangle += "add action=mark-connection chain=prerouting dst-address-type=!local new-connection-mark=cm-"
                        ...
                        + " passthrough=yes per-connection-classifier=both-addresses-and-ports:"
                        + balanceoNthBody.getLinea() + "/"
                        + +(count++) + " in-interface=" + balanceoNthBody.getInterfaceTarget() + "\n";
            }

            for (LineInterfacesBody inter : balanceoNthBody.getInterfaces()) {
                mangle += "add action=mark-routing chain=prerouting connection-mark=cm-" + inter.getWanInput()
                        + " new-routing-mark=to-" + inter.getWanInput()
                        + " passthrough=yes in-interface=" + balanceoNthBody.getInterfaceTarget() + "\n";
            }
        }

        if (balanceoNthBody.getLocal().equals("local-ip2")) {
            ...
            for (LineInterfacesBody inter : balanceoNthBody.getInterfaces()) {
                mangle += "add action=mark-connection chain=prerouting dst-address-type=!local new-connection-mark=cm-"
                        ...
                        + " passthrough=yes per-connection-classifier=both-addresses-and-ports:"
                        + balanceoNthBody.getLinea() + "/"
                        + +(count++) + " in-interface-list=" + balanceoNthBody.getInterfaceTarget() + "\n";
            }

            for (LineInterfacesBody inter : balanceoNthBody.getInterfaces()) {
                mangle += "add action=mark-routing chain=prerouting connection-mark=cm-" + inter.getWanInput()
                        + " new-routing-mark=to-" + inter.getWanInput()
                        + " passthrough=yes in-interface-list=" + balanceoNthBody.getInterfaceTarget() + "\n";
            }
        }
